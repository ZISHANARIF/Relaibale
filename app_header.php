<header>
    <div class="top-bar" bis_skin_checked="1">

        <div class="container-fluid" bis_skin_checked="1">

            <div class="row align-items-center justify-content-between g-2 ">
                <!-- Left Section: Contact -->
                <div class="col-lg-4 col-md-6 d-flex align-items-center flex-wrap ">
                    <span class="me-3 d-flex align-items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#ffffff" class="bi bi-telephone-outbound" viewBox="0 0 16 16">
                            <path d="M3.654 1.328a.678.678 0 0 0-1.015-.063L1.605 2.3c-.483.484-.661 1.169-.45 1.77a17.6 17.6 0 0 0 4.168 6.608 17.6 17.6 0 0 0 6.608 4.168c.601.211 1.286.033 1.77-.45l1.034-1.034a.678.678 0 0 0-.063-1.015l-2.307-1.794a.68.68 0 0 0-.58-.122l-2.19.547a1.75 1.75 0 0 1-1.657-.459L5.482 8.062a1.75 1.75 0 0 1-.46-1.657l.548-2.19a.68.68 0 0 0-.122-.58zM1.884.511a1.745 1.745 0 0 1 2.612.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.68.68 0 0 0 .178.643l2.457 2.457a.68.68 0 0 0 .644.178l2.189-.547a1.75 1.75 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.6 18.6 0 0 1-7.01-4.42 18.6 18.6 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877zM11 .5a.5.5 0 0 1 .5-.5h4a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-1 0V1.707l-4.146 4.147a.5.5 0 0 1-.708-.708L14.293 1H11.5a.5.5 0 0 1-.5-.5" />
                        </svg>
                        <a href="tel:+91 08048032921" class="">
                            +91 08048032921</a>
                    </span>

                    <span class="d-flex align-items-center g-2 mobile_hide">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#ffffff" class="bi bi-envelope-paper-heart" viewBox="0 0 16 16">
                            <path fill-rule="evenodd" d="M2 2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v1.133l.941.502A2 2 0 0 1 16 5.4V14a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V5.4a2 2 0 0 1 1.059-1.765L2 3.133zm0 2.267-.47.25A1 1 0 0 0 1 5.4v.817l1 .6zm1 3.15 3.75 2.25L8 8.917l1.25.75L13 7.417V2a1 1 0 0 0-1-1H4a1 1 0 0 0-1 1zm11-.6 1-.6V5.4a1 1 0 0 0-.53-.882L14 4.267zM8 2.982C9.664 1.309 13.825 4.236 8 8 2.175 4.236 6.336 1.31 8 2.982m7 4.401-4.778 2.867L15 13.117zm-.035 6.88L8 10.082l-6.965 4.18A1 1 0 0 0 2 15h12a1 1 0 0 0 .965-.738ZM1 13.116l4.778-2.867L1 7.383v5.734Z" />
                        </svg> &nbsp;
                        <a href="infosmp007@gmail.com" class="">
                            infosmp007@gmail.com</a>
                    </span>
                </div>

                <!-- Middle Section: Marquee -->
                <div class="col-lg-6 col-md-6  mobile_hide">
                    <div class="Marquee">
                        <div class="Marquee-content">
                            <p class="Marquee-tag mb-0">We are a Proprietorship owned entity which is performing its occupational activities in a proficient manner in the field of wholesaling.</p>
                        </div>
                    </div>
                </div>

                <!-- Right Section: Social Icons -->
                <div class=" mobile_hide col-lg-2 col-md-6 d-flex justify-content-lg-end justify-content-center ">
                    <ul class="social list-unstyled mb-0">
                        <li><a href="f" target="_blank" title="Facebook"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#000000" class="bi bi-facebook" viewBox="0 0 16 16">
                                    <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951" />
                                </svg></a></li>
                        <li><a href="i" target="_blank" title="Instagram"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#000000" class="bi bi-instagram" viewBox="0 0 16 16">
                                    <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.9 3.9 0 0 0-1.417.923A3.9 3.9 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.9 3.9 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.9 3.9 0 0 0-.923-1.417A3.9 3.9 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599s.453.546.598.92c.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.5 2.5 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.5 2.5 0 0 1-.92-.598 2.5 2.5 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233s.008-2.388.046-3.231c.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92s.546-.453.92-.598c.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92m-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217m0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334" />
                                </svg></a></li>
                        <li><a href="y" target="_blank" title="YouTube"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#000000" class="bi bi-youtube" viewBox="0 0 16 16">
                                    <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.01 2.01 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.01 2.01 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31 31 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.01 2.01 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A100 100 0 0 1 7.858 2zM6.4 5.209v4.818l4.157-2.408z" />
                                </svg></a></li>
                        <li><a href="x" target="_blank" title="pintrest"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#000000" class="bi bi-pinterest" viewBox="0 0 16 16">
                                    <path d="M8 0a8 8 0 0 0-2.915 15.452c-.07-.633-.134-1.606.027-2.297.146-.625.938-3.977.938-3.977s-.239-.479-.239-1.187c0-1.113.645-1.943 1.448-1.943.682 0 1.012.512 1.012 1.127 0 .686-.437 1.712-.663 2.663-.188.796.4 1.446 1.185 1.446 1.422 0 2.515-1.5 2.515-3.664 0-1.915-1.377-3.254-3.342-3.254-2.276 0-3.612 1.707-3.612 3.471 0 .688.265 1.425.595 1.826a.24.24 0 0 1 .056.23c-.061.252-.196.796-.222.907-.035.146-.116.177-.268.107-1-.465-1.624-1.926-1.624-3.1 0-2.523 1.834-4.84 5.286-4.84 2.775 0 4.932 1.977 4.932 4.62 0 2.757-1.739 4.976-4.151 4.976-.811 0-1.573-.421-1.834-.919l-.498 1.902c-.181.695-.669 1.566-.995 2.097A8 8 0 1 0 8 0" />
                                </svg></a></li>
                    </ul>
                </div>
            </div>


        </div>

    </div>



    <div class="mid-bar" bis_skin_checked="1">
        <div class="container-fluid" bis_skin_checked="1">
            <div class="d-flex justify-content-between gap-2 align-items-center" bis_skin_checked="1">
                <div class="logo" bis_skin_checked="1">
                    <a href="index.php" title="#">
                        <img src="./designer/images/logo.png" alt="lorem" title="lorem">
                    </a>
                </div>
                <div class="navigation" bis_skin_checked="1">
                    <div class="navbar" bis_skin_checked="1">
                        <button>
                            <span></span>
                            <span></span>
                            <span></span>
                            <span></span>
                        </button>
                    </div>
                    <div class="menu main_header" bis_skin_checked="1">
                        <ul class="nav">
                            <li class="active"><a href="./index.php" title="Home">Home</a></li>
                            <li><a href="" title="About Us">About</a></li>
                            <li class="ls-submenu ">
                                <a href="#" title="Our Products">Products</a>
                                <ul class="ls-dropdown">

                                    <li class="ls-submenu">
                                        <a href="#" title="BGA Rework Station">BGA Rework Station


                                        </a>
                                        <ul class="ls-dropdown">
                                            <li><a href="category.php" title="lorem ipsum">R 5860</a></li>
                                            <li><a href="#" title="lorem ipsum">R 6860</a></li>
                                        </ul>
                                    </li>

                                    <li class="ls-submenu">
                                        <a href="#" title="Fume Extractor">Fume Extractor


                                        </a>
                                        <ul class="ls-dropdown">
                                            <li><a href="category.php" title="lorem ipsum">BK 261</a></li>
                                            <li><a href="#" title="lorem ipsum">BK 262</a></li>
                                        </ul>
                                    </li>

                                   

                                    <li class="ls-submenu">
                                        <a href="#" title="ESD Tester">ESD Tester


                                        </a>
                                        <ul class="ls-dropdown">
                                            <li><a href="" title="lorem ipsum">Surface Resistivity meter</a></li>
                                            <li><a href="#" title="lorem ipsum">Esd Footwear and wrist band Tester</a></li>
                                            <li><a href="" title="lorem ipsum">Human Body Voltage Checker</a></li>
                                            <li><a href="#" title="lorem ipsum">Body static discharge dissipator</a></li>
                                            <li><a href="#" title="lorem ipsum">Esd Pole</a></li>

                                        </ul>
                                    </li>

                                    <li class="ls-submenu">
                                        <a href="#" title="ESD Tester">Soldering Station Bakon


                                        </a>
                                        <ul class="ls-dropdown">
                                            <li><a href="" title="lorem ipsum">BK3300A</a></li>
                                            <li><a href="#" title="lorem ipsum">BK1000</a></li>
                                            <li><a href="" title="lorem ipsum">BK373</a></li>
                                            <li><a href="#" title="lorem ipsum">SBK936</a></li>
                                            <li><a href="#" title="lorem ipsum">BK90</a></li>

                                        </ul>
                                    </li>

                                    <li class="ls-submenu">
                                        <a href="#" title="ESD Tester">Magnifying Lamp


                                        </a>
                                        <ul class="ls-dropdown">
                                            <li><a href="" title="lorem ipsum">5X</a></li>
                                            <li><a href="#" title="lorem ipsum">10X</a></li>
                                            <li><a href="" title="lorem ipsum">20X</a></li>
                                        </ul>
                                    </li>

                                    <li class="ls-submenu">
                                        <a href="#" title="BGA Rework Station">BGA Rework Station


                                        </a>
                                        <ul class="ls-dropdown">
                                            <li><a href="category.php" title="lorem ipsum">R 5860</a></li>
                                            <li><a href="#" title="lorem ipsum">R 6860</a></li>
                                        </ul>
                                    </li>


                                    <li class="ls-submenu">
                                        <a href="#" title="BGA Rework Station">BGA Rework Station


                                        </a>
                                        <ul class="ls-dropdown">
                                            <li><a href="category.php" title="lorem ipsum">R 5860</a></li>
                                            <li><a href="#" title="lorem ipsum">R 6860</a></li>
                                        </ul>
                                    </li>


                                    <li class="ls-submenu">
                                        <a href="#" title="BGA Rework Station">BGA Rework Station


                                        </a>
                                        <ul class="ls-dropdown">
                                            <li><a href="category.php" title="lorem ipsum">R 5860</a></li>
                                            <li><a href="#" title="lorem ipsum">R 6860</a></li>
                                        </ul>
                                    </li>

                                     <li class="ls-dropdown">
                                        <a href="#" title="lorem">ESD Chair</a>

                                    </li>





                                    <li class="ls-dropdown">
                                        <a href="#" title="lorem">Peelable Solder Mask</a>

                                    </li>

                                    <li class="ls-dropdown">
                                        <a href="#" title="lorem">Stencil Cleaning Rolls
                                        </a>

                                    </li>

                                    <li class="ls-dropdown">
                                        <a href="#" title="lorem">PCB Trolley
                                        </a>

                                    </li>


                                    <li class="ls-dropdown">
                                        <a href="#" title="lorem">Pneumatic Screw Driver Drebon
                                        </a>

                                    </li>

                                    <li class="ls-dropdown">
                                        <a href="#" title="lorem">Magnifying Lamp

                                        </a>

                                    </li>














                                </ul>
                            </li>
                            <!-- <li><a href="#" title="Faq">Products</a></li> -->

                            <li><a href="#" title="Blog">Blog</a></li>
                            <li><a href="#" title="Faq">Faq</a></li>


                            <li><a href="#" title="Contact">Gallery</a></li>

                            <li><a href="#" title="Contact">Contact</a></li>




                        </ul>
                    </div>
                </div>
                <div class="contact-list mobile_hide" bis_skin_checked="1">
                    <div class="call-now">

                        <div class="call-now mobile_hide">
                            <div class="icon-bx"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-phone-call">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                    <path d="M5 4h4l2 5l-2.5 1.5a11 11 0 0 0 5 5l1.5 -2.5l5 2v4a2 2 0 0 1 -2 2a16 16 0 0 1 -15 -15a2 2 0 0 1 2 -2"></path>
                                    <path d="M15 7a2 2 0 0 1 2 2"></path>
                                    <path d="M15 3a6 6 0 0 1 6 6"></path>
                                </svg></div>
                            <div class="con-bx">
                                <a href="tel:+91-8369201721" title="Call Us - Ultra Touch">+91-8369201721</a>
                            </div>
                        </div>

                        <button class="main-btn" data-bs-toggle="modal" data-bs-target="#sendEnquiry">Request A Quote<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chat-square-text" viewBox="0 0 16 16">
                                <path d="M14 1a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1h-2.5a2 2 0 0 0-1.6.8L8 14.333 6.1 11.8a2 2 0 0 0-1.6-.8H2a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1zM2 0a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h2.5a1 1 0 0 1 .8.4l1.9 2.533a1 1 0 0 0 1.6 0l1.9-2.533a1 1 0 0 1 .8-.4H14a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2z" />
                                <path d="M3 3.5a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5M3 6a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 3 6m0 2.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5" />
                            </svg></button>

                        <!-- <a class="main-btn2">Catalogue<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-download" viewBox="0 0 16 16">
                                <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5" />
                                <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708z" />
                            </svg></a> -->


                    </div>



                </div>
            </div>
        </div>
    </div>


</header>